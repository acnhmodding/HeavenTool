...
public class BinaryWaveFile
{
    public class BinaryWaveChannel
    {
        ...
        internal void Write(BinaryWriter writer)
        {
            ...
            writer.Write(ChannelPan);
            writer.Write(SampleRate);
            writer.Write(TotalSamplesPrefetch);
            writer.Write(TotalSamples);
            writer.Write(Coefficients);
            writeAudioOffsetNonPrefetch = writer.Position; writer.Skip(4);
            writeAudioOffset = writer.Position; writer.Skip(4);
            //writer.Write(AudioOffsetNonPrefetch);
        ...
        internal byte[] GetAudioData(BinaryReader reader)
        {
            ...
            // Is this the last channel? If so, read from the offset to the end of the stream.
            ...
            // Otherwise, read until the next channel's audio offset.
            else
            {
                int dataSize = (int) (Parent.Channels[channelIndex + 1].AudioOffset - AudioOffset);
        ...
        private static readonly int samplesPerFrame = 14;
        ...
        {
            coeffs = CalculateCoefficients(samples);
            return AdpcmEncoder.Encode(samples, coeffs);
        }

        public static short[] CalculateCoefficients(short[] source)
        {
            int frameCount = (int) Math.Ceiling((double) source.Length / samplesPerFrame);
    ...
    public byte[] ToBytes()
    {
        ...
        for (int i = 0; i < Channels.Length; i++)
        {
            ...
            channel.AudioOffset = (uint) stream.Position;

            using (writer.CreateScope())
                writer.WriteAt(channel.writeAudioOffset, channel.AudioOffset);
