    ...
    public class DarkTreeView : DarkScrollView
    {
        ...
        protected virtual bool CanMoveNodes(List<DarkTreeNode> dragNodes, DarkTreeNode dropNode, bool isMoving = false)
        {
            ...
            foreach (var node in dragNodes)
            {
                ...
                {
                    if (isMoving)
                        MessageBox.Show($"Cannot move {node.Text}. The destination folder is the same as the source folder.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                while (parentNode != null)
                {
                    ...
                    {
                        if (isMoving)
                            MessageBox.Show($"Cannot move {node.Text}. The destination folder is a subfolder of the source folder.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
