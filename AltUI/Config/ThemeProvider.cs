...
public static partial class ThemeProvider
{
    ...
    public static Color LightBackgroundColour
    {
        get
        {
    ...
    public static Color DarkBackgroundColour
    {
        get
        {
            if (TransparencyMode & WindowsVersion >= 22000)
    ...
    private static partial int DwmSetWindowAttribute(IntPtr hwnd, uint attr, int[] attrValue, int attrSize);
    ...
    public static void SetupWindow(IntPtr Handle, int Corners, bool CustomBorder = true)
    {
        ...
        DwmSetWindowAttribute(Handle, 33, [Corners], MSOTOI);
        // Set Window border to match control border
        if (CustomBorder)
            DwmSetWindowAttribute(Handle, 34, [Theme.Colors.GreySelection.ToColorRef()], MSOTOI);
        // Set Window Caption to match background
        if (WindowsVersion < 22523 || !TransparencyMode)
            DwmSetWindowAttribute(Handle, 35, [Theme.Colors.GreyBackground.ToColorRef()], MSOTOI);
        // Set Titlebar Font to match Theme
        DwmSetWindowAttribute(Handle, 36, [Theme.Colors.LightText.ToColorRef()], MSOTOI);
        // Apply immersive dark mode if it's used by system
        DwmSetWindowAttribute(Handle, 20, [LightMode ? 0 : 1], MSOTOI);
        if (TransparencyMode & WindowsVersion >= 22000)
        {
            // Mica for below 22523
            DwmSetWindowAttribute(Handle, 1029, [1], MSOTOI);
            // Mica for 22523 and up
            DwmSetWindowAttribute(Handle, 38, [LightMode ? 4 : 2], MSOTOI);
        }
    }
}
